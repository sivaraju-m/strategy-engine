...
class DailyStrategyRunner:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            for strategy_name in self.strategies:
                ...
                if strategy_config:
                    await self.strategy_coordinator.register_strategy(
    ...
    async def strategy_execution_cycle(self) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"🔄 Starting strategy execution cycle #{cycle_results['cycle_number']}"
            )
                ...
                for strategy_name in self.strategies:
                    try:
                        ...
                        signals = await self.signal_generator.generate_signals(
                            strategy_name,
                            symbol,
                            self._get_strategy_params(strategy_name),
                        )

                        if signals:
                            # Execute strategy with signals
                            execution_result = (
                                await self.strategy_coordinator.execute_strategy(
            ...
            cycle_results["execution_time"] = (
                datetime.now() - start_time
            ).total_seconds()

            # Update session statistics
            self.session_stats["cycles_completed"] += 1
            self.session_stats["signals_generated"] += cycle_results[
                "signals_generated"
            ]
            self.session_stats["strategies_executed"] += cycle_results[
                "strategies_executed"
            ]
            self.session_stats["total_execution_time"] += cycle_results[
                "execution_time"
            ]
            ...
            logger.info(
                f"✅ Cycle completed: {cycle_results['strategies_executed']} strategies, "
    ...
    async def run_daily_execution(self, duration_hours: float = 6.25) -> None:
        ...
        logger.info(
            f"🚀 Starting daily strategy execution for {duration_hours} hours..."
        )
        ...
        try:
            ...
            while self.is_running and datetime.now() < end_time:
                ...
                if (
                    self.market_validator.is_market_hours()
                    or not self.market_validator.is_market_day()
                ):
                    ...
                    if self.session_stats["cycles_completed"] % 10 == 0:
                        logger.info(
                            f"📊 Status Update - Cycles: {self.session_stats['cycles_completed']}, "
            ...
            if (
                self.market_validator.is_post_market_time()
                or datetime.now() >= end_time
            ):
...
Examples:
  ...
  python daily_strategy_runner.py --config custom.yaml   # Custom config
    ...
    parser.add_argument(
        "--extended", action="store_true", help="Extended 3-hour session"
    )
    parser.add_argument("--duration", type=float, help="Custom duration in hours")
    parser.add_argument(
        "--strategies", type=str, help="Comma-separated list of strategies"
    )
