...
class PerformanceTracker:
    ...
    async def record_execution(
        self, strategy_name: str, symbol: str, execution_result: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            strategy_stats["total_signals"] += execution_result.get(
                "signals_generated", 0
            )
            strategy_stats["total_execution_time"] += execution_result.get(
                "execution_time", 0
            )
            ...
            daily_stats["total_execution_time"] += execution_result.get(
                "execution_time", 0
            )
    ...
    def _get_strategy_breakdown(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for strategy, stats in self.strategy_performance.items():
                ...
                symbols_list = (
                    list(stats["symbols_traded"])
                    if isinstance(stats["symbols_traded"], set)
                    else stats["symbols_traded"]
                )
    ...
    def get_realtime_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_executions = sum(
                1 for exec in self.execution_metrics if exec.get("success", False)
            )
            ...
            return {
                ...
                "last_execution": (
                    self.execution_metrics[-1]["timestamp"]
                    if self.execution_metrics
                    else None
                ),
                "timestamp": datetime.now().isoformat(),
