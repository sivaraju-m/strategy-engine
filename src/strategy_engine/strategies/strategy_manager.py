...
class StrategyManager:
    ...
    def generate_combined_signals(self, data: dict[str, Any]) -> List[TradingSignal]:
        ...
        logger.info(
            f"Generated {len(filtered_signals)} combined signals from {len(self.strategies)} strategies"
        )
    ...
    def _combine_signals_for_symbol(
        ...
        combined_position_size = float(
            np.mean([s.position_size for s in relevant_signals])
        )
    ...
    def list_strategies(self) -> Dict[str, str]:
        ...
        """
        return {
            name: strategy.strategy_name for name, strategy in self.strategies.items()
        }
