...
class LSTMPredictionStrategy(BaseStrategy):
    ...
    def prepare_lstm_data(self, prices: pd.Series) -> tuple[np.ndarray, np.ndarray]:
        ...
        X, y = [], []
        for i in range(
            self.lookback_window, len(normalized_prices) - self.prediction_horizon + 1
        ):
                ...
                if abs(prediction_score) >= self.trend_threshold:
                    ...
                    if confidence >= self.min_confidence:
                        ...
                        if prediction_score > 0:
                            ...
                            reasoning = (
                                f"LSTM predicts upward trend: {prediction_score:.3f}"
                            )
                        else:
                            signal_type = SignalType.SELL
                            reasoning = (
                                f"LSTM predicts downward trend: {prediction_score:.3f}"
                            )
                        ...
                        signal = TradingSignal(
                            ...
                            risk_metrics={
                                ...
                                "trend_strength": abs(prediction_score)
                                / self.trend_threshold,
                            },
                            position_size=min(
                                0.05, abs(prediction_score) * 0.1
                            ),  # Size based on prediction
                            reasoning=reasoning,
                        )
