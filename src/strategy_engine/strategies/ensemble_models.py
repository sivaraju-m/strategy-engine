...
class RSIModel(BaseEnsembleModel):
    ...
    def fit(self, x_data: pd.DataFrame, y_data: pd.Series[Any]) -> None:
        ...
        self.is_trained = True
        logger.info(
            f"RSI model initialized: period={self.period}, OB={self.overbought}, OS={self.oversold}"
        )
...
class MomentumModel(BaseEnsembleModel):
    ...
    def fit(self, x_data: pd.DataFrame, y_data: pd.Series[Any]) -> None:
        ...
        self.is_trained = True
        logger.info(
            f"Momentum model initialized: lookback={self.lookback}, threshold={self.threshold}"
        )
    ...
    def predict(self, x_data: pd.DataFrame) -> np.ndarray:
        ...
        signals = np.where(
            momentum > self.threshold, 1, np.where(momentum < -self.threshold, -1, 0)
        )
        return signals
...
class VolumeWeightedModel(BaseEnsembleModel):
    ...
    def predict(self, x_data: pd.DataFrame) -> np.ndarray:
        ...
        signals = np.where(
            (volume_ratio > self.volume_threshold) & (price_change > 0),
            1,
            np.where(
                (volume_ratio > self.volume_threshold) & (price_change < 0), -1, 0
            ),
        )
        return signals
...
class WeightedEnsemble:
    ...
    def _hard_voting(self, predictions: Dict[str, np.ndarray]) -> Tuple[int, float]:
        ...
        weights = [
            model.weight for model in self.models if model.name in latest_predictions
        ]
...
class EnsembleStrategy:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ensemble": {"voting": "soft", "confidence_threshold": 0.6},
        }
    ...
    def backtest(
        ...
        for i in range(min_data_points, len(data)):
            ...
            if (
                signal == 1
                and position <= 0
                and confidence > self.ensemble.confidence_threshold
            ):
                # Buy signal
                if position == -1:
                    # Close short position
                    portfolio_value += (trades[-1]["price"] - current_price) * abs(
                        trades[-1]["quantity"]
                    )
            ...
            if position == 1 and trades:
                ...
                current_value = (
                    portfolio_value
                    + (trades[-1]["price"] - current_price) * trades[-1]["quantity"]
                )
            else:
        ...
        metrics = {
            ...
            "calmar_ratio": (
                total_return / abs(self._calculate_max_drawdown(pv_series))
                if self._calculate_max_drawdown(pv_series) != 0
                else 0
            ),
        }
        ...
        logger.info(
            f"Backtest completed: {total_return:.2%} return, Sharpe: {metrics['sharpe_ratio']:.3f}"
        )
    ...
    def _calculate_sharpe(
        self, returns: pd.Series[Any], risk_free_rate: float = 0.02
    ) -> float:
...
def create_sample_data(
    ...
    )  # ~0.12% daily return, 2% volatility
...
def main():
    ...
    for model_name, perf in model_perf.items():
        print(
            f"  {model_name}: Weight = {perf['weight']:.3f}, Trained = {perf['is_trained']}"
        )
