...
class MultiTimeframeStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"Initialized Multi-timeframe Strategy with timeframes: {timeframes}, "
    ...
    def calculate_indicators(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ma_fast = float(
                np.mean(close_prices[-self.ma_fast :])
                if len(close_prices) >= self.ma_fast
                else close_prices[-1]
            )
            ma_slow = float(
                np.mean(close_prices[-self.ma_slow :])
                if len(close_prices) >= self.ma_slow
                else close_prices[-1]
            )
            ...
            volatility = (
                np.std(returns[-20:]) * np.sqrt(252) if len(returns) >= 20 else 0.02
            )
            ...
            avg_volume = (
                np.mean(volume[-20:])
                if len(volume) >= 20
                else (volume[-1] if len(volume) > 0 else 0)
            )
            volume_ratio = (
                (volume[-1] / avg_volume) if avg_volume > 0 and len(volume) > 0 else 1.0
            )
            ...
            daily_trend = self._simulate_timeframe_trend(
                close_prices, 20
            )  # 20-day trend
            hourly_trend = self._simulate_timeframe_trend(
                close_prices, 5
            )  # 5-day trend
            ...
            confluence_score = self._calculate_confluence_score(
                daily_trend, hourly_trend, short_trend, price_trend
            )

            # Signal strength based on alignment
            signal_strength = self._calculate_signal_strength(
                daily_trend, hourly_trend, short_trend, rsi, volume_ratio
            )
    ...
    def _determine_trend(
        self, prices: np.ndarray, ma_fast: float, ma_slow: float
    ) -> str:
    ...
    def _calculate_confluence_score(
        self, daily: str, hourly: str, short: str, price: str
    ) -> float:
    ...
    def _calculate_signal_strength(
        self, daily: str, hourly: str, short: str, rsi: float, volume_ratio: float
    ) -> float:
    ...
    def generate_signals(self, data: Dict[str, Any]) -> List[TradingSignal]:
        ...
        try:
            ...
            if self._should_generate_buy_signal(
                daily_trend, hourly_trend, short_trend, confluence_score, rsi
            ):
            ...
            elif self._should_generate_sell_signal(
                daily_trend, hourly_trend, short_trend, confluence_score, rsi
            ):
                signal = TradingSignal(
                    symbol=symbol,
            ...
            logger.info(
                f"Generated {len(signals)} multi-timeframe signals for {symbol}"
            )
            return signals
    ...
    def _should_generate_buy_signal(
        self, daily: str, hourly: str, short: str, confluence: float, rsi: float
    ) -> bool:
        ...
        try:
            ...
            return (
                bullish_count >= self.min_confluence  # Minimum confluence
                and confluence > 65.0  # High confluence score
                and rsi < 70  # Not overbought
                and daily in ["bullish", "neutral"]  # Daily trend supportive
            )
    ...
    def _should_generate_sell_signal(
        ...
        try:
            ...
            return (
                bearish_count >= self.min_confluence  # Minimum confluence
                and confluence < 35.0  # Low confluence score (bearish)
                and rsi > 30  # Not oversold
                and daily in ["bearish", "neutral"]  # Daily trend supportive
            )
