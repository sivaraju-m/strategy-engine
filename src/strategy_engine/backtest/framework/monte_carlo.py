...
class MonteCarloSimulator:
    ...
    def simulate_strategy_performance(
        ...
        self.logger.info(
            f"Starting Monte Carlo simulation with {n_simulations} iterations"
        )
    ...
    def plot_simulation_results(
        ...
        if metric == "sharpe":
            ...
            title = (
                f"Distribution of Sharpe Ratios (Mean: {results['mean_sharpe']:.4f})"
            )
            xlabel = "Sharpe Ratio"
        elif metric == "return":
            data = results.get("total_returns", [])
            title = (
                f"Distribution of Total Returns (Mean: {results['mean_return']:.4f})"
            )
            xlabel = "Total Return"
        else:
        ...
        plt.axvline(
            np.percentile(data, 5), color="green", linestyle="dashed", linewidth=2
        )
        plt.axvline(
            np.percentile(data, 95), color="green", linestyle="dashed", linewidth=2
        )
...
class StressTester:
    ...
    def apply_market_crash(
        ...
        for i in range(
            crash_idx + 1, min(crash_idx + recovery_days + 1, len(scenario_df))
        ):
            ...
            scenario_df.iloc[
                i, scenario_df.columns.get_indexer(["open", "high", "low", "close"])
            ] *= ratio

            # Increase volume during crash (typically 2-3x)
            volume_multiplier = 3.0 if progress < 0.2 else 2.0
            scenario_df.iloc[
                i, scenario_df.columns.get_indexer(["volume"])
            ] *= volume_multiplier
    ...
    def apply_volatility_shock(
        ...
        for i in range(
            shock_idx + 1, min(shock_idx + shock_days + 1, len(scenario_df))
        ):
            ...
            scenario_df.iloc[i, scenario_df.columns.get_indexer(["high"])] = (
                new_close * (1 + high_low_range / 2)
            )
            scenario_df.iloc[i, scenario_df.columns.get_indexer(["low"])] = (
                new_close * (1 - high_low_range / 2)
            )
            scenario_df.iloc[i, scenario_df.columns.get_indexer(["open"])] = (
                prev_close * (1 + random_return / 2)
            )

            # Increase volume during volatility shock
            scenario_df.iloc[i, scenario_df.columns.get_indexer(["volume"])] *= (
                1.5 + np.abs(random_return) * 10
            )
    ...
    def apply_liquidity_crisis(
        ...
        for i in range(
            crisis_idx + 1, min(crisis_idx + crisis_days + 1, len(scenario_df))
        ):
            # Reduce volume
            scenario_df.iloc[i, scenario_df.columns.get_indexer(["volume"])] *= (
                1 - volume_reduction
        ...
        for scenario in scenarios:
            if scenario == "market_crash":
                scenario_df = self.apply_market_crash(
                    df,
                    crash_percent=default_scenario_params["market_crash"][
                        "crash_percent"
                    ],
                    recovery_days=default_scenario_params["market_crash"][
                        "recovery_days"
                    ],
                )
                ...
                scenario_df = self.apply_volatility_shock(
                    df,
                    volatility_multiplier=default_scenario_params["volatility_shock"][
                        "volatility_multiplier"
                    ],
                    shock_days=default_scenario_params["volatility_shock"][
                        "shock_days"
                    ],
                )
                ...
                scenario_df = self.apply_liquidity_crisis(
                    df,
                    volume_reduction=default_scenario_params["liquidity_crisis"][
                        "volume_reduction"
                    ],
                    spread_multiplier=default_scenario_params["liquidity_crisis"][
                        "spread_multiplier"
                    ],
                    crisis_days=default_scenario_params["liquidity_crisis"][
                        "crisis_days"
                    ],
                )
            else:
            ...
            try:
                ...
                # Calculate impact
                impact = {
                    "sharpe_impact": stress_metrics["sharpe_ratio"]
                    - baseline_metrics["sharpe_ratio"],
                    "return_impact": stress_metrics["total_return"]
                    - baseline_metrics["total_return"],
                    "drawdown_impact": stress_metrics["max_drawdown"]
                    - baseline_metrics["max_drawdown"],
                    "win_rate_impact": stress_metrics["win_rate"]
                ...
                self.logger.info(
                    f"  Baseline Return: {baseline_metrics['total_return']:.4f}, "
    ...
    def _calculate_metrics(
        self, df: pd.DataFrame, signals: List[float]
    ) -> Dict[str, float]:
        """
    ...
    def bootstrap_test(
        ...
        for _ in range(self.n_bootstrap):
            ...
            bootstrap_sharpes.append(
                np.sqrt(252) * strategy_sample.mean() / strategy_sample.std()
                if strategy_sample.std() > 0
                else 0
            )
            ...
            if benchmark_returns is not None:
                ...
                bootstrap_alphas.append(
                    strategy_sample.mean() - benchmark_sample.mean()
                )
                bootstrap_betas.append(
                    np.cov(strategy_sample, benchmark_sample)[0, 1]
                    / np.var(benchmark_sample)
                    if np.var(benchmark_sample) > 0
                    else 0
                )
        ...
        # Add interpretations
        results["mean_return"]["significant"] = results["mean_return"]["p_value"] < (
            1 - self.confidence_level
        )
        results["sharpe_ratio"]["significant"] = results["sharpe_ratio"]["p_value"] < (
            1 - self.confidence_level
        )

        if benchmark_returns is not None:
            results["alpha"]["significant"] = results["alpha"]["p_value"] < (
                1 - self.confidence_level
            )
        ...
        self.logger.info(
            f"  Mean Return: {actual_mean:.6f}, p-value: {results['mean_return']['p_value']:.4f}, "
            f"Significant: {results['mean_return']['significant']}"
        )
        self.logger.info(
            f"  Sharpe Ratio: {actual_sharpe:.4f}, p-value: {results['sharpe_ratio']['p_value']:.4f}, "
            f"Significant: {results['sharpe_ratio']['significant']}"
        )

        if benchmark_returns is not None:
            self.logger.info(
                f"  Alpha: {actual_alpha:.6f}, p-value: {results['alpha']['p_value']:.4f}, "
