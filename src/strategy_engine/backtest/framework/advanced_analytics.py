...
class StressTester:
    ...
    def _apply_market_crash(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(crash_duration):
            if crash_start + i < len(result):
                idx = crash_start + i
                result.loc[result.index[idx], 'close'] = result.iloc[idx-1]['close'] * daily_factor
                
                # Adjust other price columns
                for col in ['open', 'high', 'low']:
                    if col in result.columns:
                        ratio = df.iloc[idx][col] / df.iloc[idx]['close']
                        result.loc[result.index[idx], col] = result.iloc[idx]['close'] * ratio
        
        # Propagate the effect to the rest of the data
        final_crash_idx = crash_start + crash_duration - 1
        if final_crash_idx < len(result):
            crash_ratio = result.iloc[final_crash_idx]['close'] / df.iloc[final_crash_idx]['close']
    ...
    def _apply_high_volatility(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(vol_start, min(vol_end, len(result))):
            if i > 0:
                ...
                result.loc[result.index[i], 'close'] = result.iloc[i-1]['close'] * (1 + amplified_return)
                
                # Adjust other price columns
                for col in ['open', 'high', 'low']:
                    if col in result.columns:
                        ratio = df.iloc[i][col] / df.iloc[i]['close']
                        result.loc[result.index[i], col] = result.iloc[i]['close'] * ratio
    ...
    def _apply_trending_market(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'close' in result.columns:
            ...
            for i in range(trend_start + 1, len(result)):
                ...
                result.loc[result.index[i], 'close'] = trend_price * 0.7 + (base_price * orig_ratio * 0.3)
        ...
        if 'close' in result.columns:
            ...
            for i in range(choppy_start + 1, len(result)):
                ...
                if i > 0:
                    ...
                    result.loc[result.index[i], 'close'] = result.iloc[i-1]['close'] * (1 + blended_change)
    ...
    def run_strategy_stress_test(
        ...
        baseline_df['strategy_return'] = baseline_df['signal'].shift(1) * baseline_df['return']
        
        baseline_returns = baseline_df['strategy_return'].dropna()
        
        baseline_metrics = {
            'total_return': (1 + baseline_returns).prod() - 1 if len(baseline_returns) > 0 else 0,
            'sharpe_ratio': np.sqrt(252) * baseline_returns.mean() / baseline_returns.std() if len(baseline_returns) > 0 and baseline_returns.std() > 0 else 0,
            'max_drawdown': (baseline_df['close'] / baseline_df['close'].cummax() - 1).min() if len(baseline_df) > 0 else 0,
            'win_rate': (baseline_returns > 0).mean() if len(baseline_returns) > 0 else 0
        }
        ...
        for scenario in scenarios_to_run:
            ...
            scenario_df['strategy_return'] = scenario_df['signal'].shift(1) * scenario_df['return']
            
            scenario_returns = scenario_df['strategy_return'].dropna()
            
            scenario_metrics = {
                'total_return': (1 + scenario_returns).prod() - 1 if len(scenario_returns) > 0 else 0,
                'sharpe_ratio': np.sqrt(252) * scenario_returns.mean() / scenario_returns.std() if len(scenario_returns) > 0 and scenario_returns.std() > 0 else 0,
                'max_drawdown': (scenario_df['close'] / scenario_df['close'].cummax() - 1).min() if len(scenario_df) > 0 else 0,
                'win_rate': (scenario_returns > 0).mean() if len(scenario_returns) > 0 else 0
            }
            
            # Calculate impact compared to baseline
            impact = {
                'return_impact': scenario_metrics['total_return'] - baseline_metrics['total_return'],
                'sharpe_impact': scenario_metrics['sharpe_ratio'] - baseline_metrics['sharpe_ratio'],
                'drawdown_impact': scenario_metrics['max_drawdown'] - baseline_metrics['max_drawdown'],
                'win_rate_impact': scenario_metrics['win_rate'] - baseline_metrics['win_rate']
            }
...
class StatisticalSignificanceTester:
    ...
    def t_test(
        ...
        if len(strategy_returns) < 30:
            return {'error': 'Insufficient data for t-test (minimum 30 observations required)'}
    ...
    def bootstrap_test(
        ...
        """
        if len(strategy_returns) < 30:
            return {'error': 'Insufficient data for bootstrap test (minimum 30 observations required)'}
        ...
        for _ in range(n_iterations):
            ...
            bootstrap_sample = strategy_returns.sample(len(strategy_returns), replace=True)
            
            # Calculate metrics
            mean_return = bootstrap_sample.mean()
            sharpe_ratio = np.sqrt(252) * mean_return / bootstrap_sample.std() if bootstrap_sample.std() > 0 else 0
        ...
        results = {
            ...
            'sharpe_ratio': {
                'value': np.sqrt(252) * strategy_returns.mean() / strategy_returns.std() if strategy_returns.std() > 0 else 0,
        ...
        if benchmark_returns is not None and len(benchmark_returns) == len(strategy_returns):
            ...
            for _ in range(n_iterations):
                ...
                indices = np.random.choice(len(strategy_returns), len(strategy_returns), replace=True)
