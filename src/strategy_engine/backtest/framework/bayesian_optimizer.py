...
class BayesianStrategyOptimizer:
    ...
    def _objective_function(self, **params):
        ...
        if self.results_table:
            metrics = {
                ...
                'timestamp': datetime.now(),
                'symbol': self.optimization_data['symbol'].iloc[0] if 'symbol' in self.optimization_data.columns else 'unknown',
    ...
    def optimize(
        ...
        if verbose:
            print(f"Starting Bayesian optimization for {self.strategy_class.__name__}...")
        ...
        optimal_params = {name: value for name, value in zip(self.param_names, result.x)}
    ...
    def plot_optimization_results(
        ...
        try:
            ...
            if output_dir:
                plt.savefig(f"{output_dir}/convergence_{self.strategy_class.__name__}.png")
                plt.close()
            else:
                plt.show()
            ...
            if 'parameter_importance' in result:
                ...
                if output_dir:
                    plt.tight_layout()
                    plt.savefig(f"{output_dir}/importance_{self.strategy_class.__name__}.png")
                    plt.close()
                else:
                    plt.tight_layout()
                    plt.show()
            ...
            print("Plotting requires matplotlib and scikit-optimize. Install with: pip install matplotlib scikit-optimize")
