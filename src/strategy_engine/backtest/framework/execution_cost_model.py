...
class TransactionCostModel:
    ...
    def calculate_cost(
        ...
        if self.broker_type == BrokerType.CUSTOM:
            ...
            fees = self.fee_structures.get(self.broker_type, self.fee_structures[BrokerType.ZERODHA])
        ...
        if self.include_stt:
            if is_delivery:
                stt = trade_value * (self.stt_buy_delivery if is_buy else self.stt_sell_delivery)
            else:
...
class RealisticExecutionModel:
    ...
    def calculate_realistic_execution(
        ...
        total_impact = (final_value - initial_value) / initial_value if initial_value > 0 else 0
    ...
    def apply_execution_costs_to_backtest(
        ...
        for i in range(len(signals)):
            ...
            if signal != 0 or current_position != 0:
                ...
                slippage_impact = execution_result['slippage']['slippage_percentage'] / 100
        ...
        original_sharpe = np.mean(original_returns) / np.std(original_returns) * np.sqrt(252) if np.std(original_returns) > 0 else 0
        adjusted_sharpe = np.mean(adjusted_returns) / np.std(adjusted_returns) * np.sqrt(252) if np.std(adjusted_returns) > 0 else 0
        ...
        return {
            ...
            'original_total_return': original_cumulative[-1] - 1 if len(original_cumulative) > 0 else 0,
            'adjusted_total_return': adjusted_cumulative[-1] - 1 if len(adjusted_cumulative) > 0 else 0,
