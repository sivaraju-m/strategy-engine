...
class MarketRegimeClassifier:
    ...
    def classify(
        self, df: pd.DataFrame, index_data: Optional[pd.DataFrame] = None
    ) -> pd.DataFrame:
        """
        ...
        data["volatility"] = data["pct_change"].rolling(
        ...
        result = df.copy()
        result = pd.merge(result, data[["date", "regime"]], on="date", how="left")
    ...
    def get_regime_periods(
        ...
        for i in range(len(regime_start_indices)):
            ...
            end_idx = (
                regime_start_indices[i + 1] - 1
                if i < len(regime_start_indices) - 1
                else df.index[-1]
            )
        ...
        for regime in regimes:
            ...
            for i in range(len(regime_data) - 1):
                ...
                if result and "signal" in result:
                    ...
                    if signal != 0:
                        price_change = (
                            next_row["close"].values[0] - row["close"].values[0]
                        ) / row["close"].values[0]
                        trade_return = (
                            price_change * signal
    ...
    def run_multi_regime_comparison(
        ...
        for symbol in symbols:
            ...
            for regime, metrics in symbol_results.items():
                ...
                all_results[regime]["sharpe_ratios"].append(
                    metrics.get("sharpe_ratio", 0)
                )
                all_results[regime]["total_returns"].append(
                    metrics.get("total_return", 0)
                )
                all_results[regime]["max_drawdowns"].append(
        ...
        for regime, data in all_results.items():
            data["avg_sharpe_ratio"] = (
                np.mean(data["sharpe_ratios"]) if data["sharpe_ratios"] else 0
            )
            data["avg_total_return"] = (
                np.mean(data["total_returns"]) if data["total_returns"] else 0
            )
            data["avg_max_drawdown"] = (
                np.mean(data["max_drawdowns"]) if data["max_drawdowns"] else 0
            )
            data["avg_win_rate"] = (
                np.mean(data["win_rates"]) if data["win_rates"] else 0
            )
