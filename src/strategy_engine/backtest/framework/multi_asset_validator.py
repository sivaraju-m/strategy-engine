...
class MultiAssetValidator:
    ...
    def get_symbols_by_market_cap(
        ...
        if cap_segment not in valid_segments:
            raise ValueError(f"Invalid market cap segment. Must be one of {valid_segments}")
    ...
    def validate_across_market_caps(
        ...
        for segment in cap_segments:
                    ...
                    for i in range(len(df) - 1):
                        try:
                            ...
                            if result and 'signal' in result:
                                ...
                                trade_return = next_return * signal if not np.isnan(next_return) else 0
            ...
            # Calculate averages
            results[segment] = {
                'avg_sharpe_ratio': np.mean(segment_results['sharpe_ratios']) if segment_results['sharpe_ratios'] else 0,
                'avg_total_return': np.mean(segment_results['total_returns']) if segment_results['total_returns'] else 0,
                'avg_max_drawdown': np.mean(segment_results['max_drawdowns']) if segment_results['max_drawdowns'] else 0,
                'avg_win_rate': np.mean(segment_results['win_rates']) if segment_results['win_rates'] else 0,
    ...
    def validate_across_sectors(
        ...
        for sector in sectors:
            ...
            # Calculate averages
            results[sector] = {
                'avg_sharpe_ratio': np.mean(sector_results['sharpe_ratios']) if sector_results['sharpe_ratios'] else 0,
                'avg_total_return': np.mean(sector_results['total_returns']) if sector_results['total_returns'] else 0,
                'avg_max_drawdown': np.mean(sector_results['max_drawdowns']) if sector_results['max_drawdowns'] else 0,
                'avg_win_rate': np.mean(sector_results['win_rates']) if sector_results['win_rates'] else 0,
